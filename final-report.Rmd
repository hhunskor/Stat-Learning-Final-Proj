...
Rather than use all 79 variables from the dataset for our analysis, we decided to select 23 variables--7 categorical, 6 continuous and 10 numeric--in this report that we found particularly interesting. The variables that we have decided to use in this report is listed below.
...
- YearRemodAdd: Year house was remodelled
...
var <- numeric_var[-c(1:3,9:12,14:16,18:19,24, 26:27,29:33,35:36)]
...
cat_var<-categorical_var[c(9,12:13,25,29,35,38)]
...
sd_year <- sd(classification_dat$YearBuilt)
mean_year <- mean(classification_dat$YearBuilt)
```
...
#for (i in 1:(nrow(train_set)/2)){
for (i in 1:100){
knn_preds <- knn(train = classification_dat_scale%>%dplyr::select(-Neighborhood),
                 test = classification_dat_scale%>% dplyr::select(-Neighborhood),
                 cl =classification_dat_scale$Neighborhood, 
                 k = i)
#table(pred = knn_preds, truth = test_set$Neighborhood)
misclassification[i]<-mean(classification_dat_scale$Neighborhood != knn_preds)}


```
```{r}
knn_result<-data.frame (K  = 1:100,
            missclassification_rate= misclassification)
knn_result  %>%
  ggplot(., aes(x = K, y = missclassification_rate)) +
  geom_point() 
...
```
**Need to check if pruning makes it better**
...
To answer our first research question relating to the regression problem, "Which statistical learning method most accurately predicts housing prices using other available housing features", we use three different regression models: Linear Regression, Ridge Regression and Lasso Regression. We found that the Root Mean Squared Error (RMSE) was 24588.56 for simple linear regression model, 25638.98. for ridge regression model and 25077.4. for the lasso model. This implies that for the AMes housing dataset, the simple linear regression model performed the best. This was a little surprising to us as we expected that the lasso model might fit better as it shrinks features that are not has important to 0. However, after giving some though we realized that the lasso model did not really shrink many variables to 0 in our model, implying that most of the variables that we used in our model are important features in determining the sale price of the houses.Thus, it makes sense why the linear model performed the best. Overal, we think that the our best performing model is a good choice to predict the sale prices as RMSE of 25077.4 is low if we compare it to the sale price of houses, which are much higher.
