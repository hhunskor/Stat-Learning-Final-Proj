...
```{r}
#Not sure if we need to include this output in the report just because it will take up a ton of space
summary(dat)
lapply(dat,class)
```
...
Rather than use all 79 variables from the dataset for our analysis, we decided to select 17 variables that we found particularly interesting -- 4 categorical, 6 continuous and 7 numeric. 

Below is a brief description of the variables that we have decided to use:
