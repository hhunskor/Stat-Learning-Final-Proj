...
his is the smallest test RMSE we've seen so far.
...
The misclassification rate of ~ 0.64 is significantly better compared to the missclassification rate observed using Naive Bayes. Having said that, misclassification rate of ~ 0.46 is still quite high. So we decided to check whether pruning the tree will improve the results.
...
Pruning increased the misclassification slightly from ~ 0.46 to ~ 0.48.
...
To summarize the results of our classification models, we found the misclassification rates of the Naive Bayes and Decision Tree models to be approximately .64 and .46, respectively. Although the tree-based method performs significantly better, we still think that the misclassification rate is really high. This suggests that perhaps the neighborhoods are not clustered based solely on the predictors we used.
